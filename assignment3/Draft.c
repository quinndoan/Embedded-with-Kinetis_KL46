// đã đúng phần gửi sang UART và đã gửi được từ ADC, chưa kiểm tra số đã đúng chưa và chưa chuyển đổi sang nhiệt độ
...
void PIT_IRQHandler(void) {
    if (PIT->CHANNEL[0].TFLG & PIT_TFLG_TIF_MASK) {
        ...
        ADC0->SC1[0] = ADC_SC1_ADCH(26); // Correct ADC channel
    }
}
