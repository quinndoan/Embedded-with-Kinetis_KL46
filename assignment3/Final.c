...
void UART_TransmitValue(uint16_t value){
    ...
    float voltage = (value * V_REF) / ADC_MAX;
    float temperatureFloat = 25.0f - ((voltage - V_TEMP25) / M);
    uint16_t temperature = (uint16_t)temperatureFloat; // Convert to unsigned integer

    // Convert temperature to string and transmit via UART
    char buffer[10];
