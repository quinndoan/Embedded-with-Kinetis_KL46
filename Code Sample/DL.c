...
//				{
//					Status = Button_1;
//					while((GPIOC->PDIR & (1U << 3)) == 0)
//					{
//					}
//				}
//			}
//			// Button 2
//	else if ((GPIOC->PDIR & (1U << 12)) == 0 )
//			{
//				delay(20);
...
// Frame
	...
	// enable TE, RE to write UART registers
//	UART0->C2 |= UART0_C2_TE_MASK;
//	UART0->C2 |= UART0_C2_RE_MASK;
...
//// configure Button
//	// switch 1
//	PORTPin_Init(PORTC, 3U, &PortBTNConfig);
//	//switch 2
//	PORTPin_Init(PORTC, 12U, &PortBTNConfig);
// configure output for led and input for buttons
	GPIOD ->PDDR |= (1U << 5); // green Led
	GPIOE ->PDDR |= (1U << 29); // red Led
//	GPIOD -> PDOR |= (1U << 5);
//	GPIOC ->PDDR |= (1U << 3);
//	GPIOC ->PDDR &= ~(1U << 3); // button 1
//	GPIOC ->PDDR &= ~ (1U << 12); // button 2

	// turn off 2 LEDs
//	GPIOD -> PDOR |= (1U << 5); // turn off green one
//	GPIOE -> PDOR |= (1U << 29); // turn off red one


//	uint8_t idx;
//	Status_t ButtonPress;
//	uint16_t Data ;
//	ADC_unit();// HW trigger
	// init PIT
//	Data = ADC_Readpoll();
...
//	SIM->SOPT7 |= SIM_SOPT7_ADC0TRGSEL(4U);
//	SIM->SOPT7 |= SIM_SOPT7_ADC0ALTTRGEN(1U);
	// select ADC channel
//	ADC0 -> SC1[0] = (ADC0 ->SC1[0] & ~ADC_SC1_ADCH_MASK) | ADC_SC1_ADCH(3U);

//	PIT_StartTimer();
	// convert to V = Data * 3V / (2^16 -1)
// enable in runtime SysTick
//	Systick_Init100ms();
//	SysTick->CTRL |= (1U << SysTick_CTRL_ENABLE_Pos);
	UART_Init();
//	uint8_t str[5] ="ABCDEFG";
