...
volatile uint8_t rxBuffer1[BUFFER_SIZE];
volatile uint8_t rxBuffer2[BUFFER_SIZE];
volatile uint8_t activeBufferIndex = 0;
// 0 cho Buffer 1 và 1 cho Buffer 2

volatile uint8_t rxIndex1 = 0;
volatile uint8_t rxIndex2 = 0;
// Thêm idex cho Buffer1 và Buffer2

volatile uint8_t procBuffer[BUFFER_SIZE]; // Add Processing buffer
volatile uint8_t procIndex = 0; // Processing buffer index
volatile bool procReady = false; // Check if processing buffer is ready or not
...
// Hàm Interrupt này giúp chạy được 1 số dòng đúng, nhưng vẫn bị ghi đè
    ...
    if (UART0->S1 & UART0_S1_RDRF_MASK) {
        ...
        if (receivedChar != '\r') {
            if (activeBufferIndex == 0) {
                ...
                if (rxIndex1 >= BUFFER_SIZE || receivedChar == '\n') {
                    newLine = true;
                    memcpy((char *)procBuffer, (char *)rxBuffer1, rxIndex1);
                    procIndex = rxIndex1;
                    rxIndex1 = 0;
                    procReady = true;
                    bufferSwitch = true;
                ...
                rxBuffer2[rxIndex2++] = receivedChar;
                if (rxIndex2 >= BUFFER_SIZE || receivedChar == '\n') {
                    newLine = true;
                    memcpy((char *)procBuffer, (char *)rxBuffer2, rxIndex2);
                    procIndex = rxIndex2;
                    rxIndex2 = 0;
                    procReady = true;
                    bufferSwitch = true;
