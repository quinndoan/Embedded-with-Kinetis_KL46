...
int main(void) {
    ...
    const char *srecData[] = {
        "S21402040000600020D50402004305020045050200F4",
		"S21402041000000000000000000000000000000000D5",
		"S2140204200000000000000000000000004705020077",
		NULL // Đánh dấu kết thúc
    };

    // Đưa các dòng SREC vào hàng đợi
    for (const char **line = srecData; *line != NULL; line++) {
        Queue_Enqueue(&srecQueue, (char *)*line, strlen(*line));
    }

    // Main loop để dequeue và xử lý các dòng SREC
    while (1) {
        QueueItem item;
        if (Queue_Dequeue(&srecQueue, &item)) {
            Process_SREC(item.data);
        } else {
            break; // Thoát vòng lặp khi hàng đợi trống
        }
    }
